...
class CompletedTransactionTile extends StatefulWidget {
  final local.Transaction transaction;

  const CompletedTransactionTile({Key? key, required this.transaction})
      : super(key: key);

  @override
  _CompletedTransactionTileState createState() =>
      _CompletedTransactionTileState();
}

class _CompletedTransactionTileState extends State<CompletedTransactionTile> {
      ...
      builder: (context, snapshot) {
        ...
        if (snapshot.hasData) {
          ...
          tile = ListTile(
            ...
            subtitle: Text(
                "${_debtorName == 'you' ? 'You' : _debtorName} ${_debtorName == 'you' ? 'owe' : 'owes'} $_creditorName ${widget.transaction.amount} ${widget.transaction.currency}"),
            trailing: IconButton(
